...
std::vector<int> GameState::get_board() const { return board_; }
...
int GameState::get_cell(position pos) const {
    if (!is_valid_cell(pos)) return NO_PLAYER;
    return board_[3 * pos.y + pos.x];
}

void GameState::set_cell(position pos, int player_id) {
    if (is_valid_cell(pos)) board_[3 * pos.y + pos.x] = player_id;
}
...
int GameState::get_current_player() const { return current_player_; }
