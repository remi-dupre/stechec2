...
int ActionPlay::check(const GameState& st) const {
    if (!st.is_valid_cell(pos_)) return OUT_OF_BOUNDS;
    if (st.get_cell(pos_) != st.NO_PLAYER) return ALREADY_OCCUPIED;
    if (!st.player_can_play(player_id_)) return ALREADY_PLAYED;
