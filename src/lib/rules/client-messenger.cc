...
void ClientMessenger::send(const utils::Buffer& buf) {
    ...
    if (!sckt_->send(out_buf)) FATAL("Unable to send message to server");
}
...
std::unique_ptr<utils::Buffer> ClientMessenger::recv() {
    auto buf = sckt_->recv();
    if (!buf) FATAL("Unable to receive message from server");
...
void ClientMessenger::pull_id(uint32_t* id) { pull()->handle(*id); }
...
void ClientMessenger::ack() {
    ...
    if (!sckt_->send(buf)) FATAL("Unable to send ack to server");
}

void ClientMessenger::wait_for_ack() {
    auto buf = sckt_->recv();
    if (!buf) FATAL("Unable to receive ack from server");
