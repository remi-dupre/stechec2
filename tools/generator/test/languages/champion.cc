...
void test() {
    ...
    for (int i = 1; i < 100; ++i) assert(r[i - 1] == i);

    r = returns_range(1, 10000);
    for (int i = 1; i < 10000; ++i) assert(r[i - 1] == i);

    std::vector<int> v = {1, 3, 2, 4, 5, 7, 6};
    v = returns_sorted(v);
    for (int i = 0; i < 7; ++i) assert(v[i] == i + 1);
    ...
    ba = returns_not(ba);
    for (int i = 0; i < 9; ++i) assert(ba[i] == (i % 3 != 0));
