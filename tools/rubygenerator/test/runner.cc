...
void api_send_me_42s(struct_with_array s) {
    ...
    for (auto i : s.field_int_arr) assert(i == 42);
...
std::vector<struct_with_array> api_send_me_struct_array(
    std::vector<struct_with_array> l) {
    assert(l.size() == 42);
    for (auto& s : l) api_send_me_42s(s);
    return l;
}
}
...
int main(int argc, char* argv[]) {
    int count = 10000;
    if (argc > 1) count = std::strtol(argv[1], NULL, 0);
